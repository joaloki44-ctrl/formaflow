...
export async function POST(req: Request) {
  try {
    ...
    if (!lessonId || !courseId) return new NextResponse("Données manquantes", { status: 400 });

    const enrollment = await prisma.enrollment.findFirst({
      where: { userId: user.id, courseId: courseId },
    });

    if (!enrollment) return new NextResponse("Non inscrit", { status: 403 });
    ...
    const totalLessons = await prisma.lesson.count({
      where: { module: { courseId: courseId }, isPublished: true },
    });
