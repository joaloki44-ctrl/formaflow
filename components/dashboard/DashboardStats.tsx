...
export default function DashboardStats({ totalCourses, totalStudents, totalRevenue, growth }: DashboardStatsProps) {
  const stats = [
    ...
    { label: "Croissance", value: growth, icon: TrendingUp, color: "text-amber-600 bg-amber-50" },
  ];
      ...
      {stats.map((stat) => {
        ...
        return (
          ...
          >
            ...
            <div className="absolute top-0 right-0 w-24 h-24 bg-gray-50/50 rounded-full translate-x-1/2 -translate-y-1/2 group-hover:bg-secondary/5 transition-colors" />
          </div>
        );
      })}
    </div>
